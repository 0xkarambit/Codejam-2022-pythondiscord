...
class EventHandler:
    ...
    async def searching(self, uid, socket):
        ...
        if len(self.current_room) == 4:
            ...
            self.active_rooms[room_uid] = {{i: {"position x": 0, "position y": 0, "is_dead": False, "animation_state": "idle"}} for i in self.current_room}
    ...
    async def set_data(self, uid, position1, position2, is_dead, animation_state, socket):
...
async def handle_request(request: str, websocket):
    ...
    match req:
        ...
        case {"id": UID, "data": [position1, position2, is_dead, animation_state]}:
            await event_handler.set_data(UID, position1, position2, is_dead, animation_state, websocket)
